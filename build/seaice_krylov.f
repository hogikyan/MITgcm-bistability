...
C
      ...
      Real*8 uIceLin(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vIceLin(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     extra time level required for backward difference time stepping
      Real*8 duIcNm1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 dvIcNm1(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     u/vWork   :: work arrays
      Real*8 uWork  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vWork  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     helper array
      Real*8 resTmp (nVec,1,nSx,nSy)
C     work arrays
      Real*8 rhs(nVec,nSx,nSy), sol(nVec,nSx,nSy)
      Real*8 vv(nVec,im+1,nSx,nSy), w(nVec,im,nSx,nSy)
      Real*8 wk1(nVec,nSx,nSy), wk2(nVec,nSx,nSy)
CEOP
...
C     backward difference extrapolation factors
      ...
      IF ( SEAICEuseBDF2 ) THEN
       ...
       ENDIF
      ENDIF
      bdfAlpha = 1.D0 + bdfFac

      DO bj=myByLo(myThid),myByHi(myThid)
       DO bi=myBxLo(myThid),myBxHi(myThid)
        DO J=1-OLy,sNy+OLy
         DO I=1-OLx,sNx+OLx
          uIceLHS(I,J,bi,bj) = 0.D0
          vIceLHS(I,J,bi,bj) = 0.D0
          uIceRHS(I,J,bi,bj) = 0.D0
          vIceRHS(I,J,bi,bj) = 0.D0
         ENDDO
        ENDDO
...
C     the non-linear drag coefficents
       DO bj=myByLo(myThid),myByHi(myThid)
        DO bi=myBxLo(myThid),myBxHi(myThid)
         DO j=1-OLy,sNy+OLy
          DO i=1-OLx,sNx+OLx
           uIceLin(I,J,bi,bj) = 0.5D0 *
     &          (uIce(I,J,bi,bj) + uIceLin(I,J,bi,bj))
           vIceLin(I,J,bi,bj) = 0.5D0 *
...
C     start Krylov iteration (FGMRES)
        ...
        DO bj=myByLo(myThid),myByHi(myThid)
         DO bi=myBxLo(myThid),myBxHi(myThid)
          DO j=1-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
            uWork(i,j,bi,bj) = 0.D0
            vWork(i,j,bi,bj) = 0.D0
           ENDDO
          ENDDO
         ENDDO
        ENDDO
...
C--   Output diagnostics
      ...
      IF ( SEAICE_monFreq .GT. 0.D0 ) THEN
