...
C This allows to use Visbeck et al formulation to compute K_GM+Redi !cyc: turn on
...
C     for Visbeck et al. parameterization
      Real*8 VisbeckK(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     !LOCAL VARIABLES:
      ...
      Real*8 dSigmaDrW
      Real*8 dSigmaDrS
      Real*8 wkb_cW(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 wkb_cS(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 rPI, c2
      Real*8 tmpFac
      CHARACTER*(MAX_LEN_MBUF) msgBuf

      PARAMETER ( rPI    = 0.318309886183791D0 )

C-    Matrix elements for tri-diagonal solver
      Real*8 GM_a3d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8 GM_b3d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8 GM_c3d(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
...
C      Surface BC : set to zero
       DO j=1-OLy,sNy+OLy
        DO i=1-OLx,sNx+OLx
          GM_PsiX(i,j,1,bi,bj) = 0.D0
          GM_PsiY(i,j,1,bi,bj) = 0.D0
        ENDDO
       ENDDO
