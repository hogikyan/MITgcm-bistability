...
CBOP
C     This file was hacked by Malte Jansen to artificially reduce sensible
C     heat transfer (hs) to a simple restoring condition
C     (notice that latent heat transfer can be easily turned off by
C     setting Dalton number to zero, and radiative flux can be turned off by
C     setting ocean emissivity, lwdown and swdown to zero)
C     although if setting emissivity to zero we also need to turn off
C     an error catch in seaice_readparms.F
...
C     !cyc: define useSensFluxRestore and TauSensFluxRestore
      LOGICAL useSensFluxRestore
      Real*8     TauSensFluxRestore
...
C                surface temperature (in deg Celsius)
      ...
      INTEGER myIter
      INTEGER myThid
      Real*8 exf_Tsf(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     output:
CEOP
...
C     -   Turbulent Fluxes
           IF ( useSensFluxRestore ) THEN
C          !cyc : write sensible hear flux as restoring flux              
              hs(i,j,bi,bj) = TauSensFluxRestore*deltap(i,j)
           ELSE
