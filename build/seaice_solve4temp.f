...
C #if ( defined USE_EXF_INTERPOLATION && defined  && defined USING_THREADS )

C-  Not recommended (not tested nor maintained) and un-documented Options:


CBOP
C     This file has been modified by Malte Jansen to
C     effectively implement a simple restoring condition.
C     This is a pretty dirty hack, and should maybe be implemented
C     as a proper option at some point.
C     What is done is turn off sublimation (by setting D1I=0), turning off lwflux
C     and editing the sens heat flux to a simple restoring
C     Notice that evap over open ocean, which is generally also part of the
C     ice package is not being used because EXF package is used.
C     Notice that we also need to set MIN_LWDOWN = 0.0
...
C     ICE LATENT HEAT CONSTANT
      lhSublim = SEAICE_lhEvap + SEAICE_lhFusion
       IF ( SEAICEuseTotFluxRestore ) THEN
C     !cyc: set to zero
          D1I=0.D0*SEAICE_dalton*lhSublim*SEAICE_rhoAir
       ELSE
...
C     Calculate the flux terms based on the updated tsurfLoc
          ...
          F_lwu(I,J) = t4 * D3(I,J)
          IF ( SEAICEuseTotFluxRestore ) THEN
             F_sens(I,J)= SEAICE_taufluxRest * (t1 - atempLoc(I,J))
             F_ia(I,J) = F_sens(I,J)
             dFia_dTs(I,J) = SEAICE_taufluxRest
          ELSE
...
C     over ice specific humidity
          ...
          F_lh(I,J) = D1I * UG(I,J)*(qhice(I,J)-AQH(I,J,bi,bj))
          F_lwu(I,J)  = t4 * D3(I,J)
C     !cyc: set total flux as sensible flux as a restoring flux
          IF ( SEAICEuseTotFluxRestore ) THEN
             F_sens(I,J)= SEAICE_taufluxRest * (t1 - atempLoc(I,J))
             F_ia(I,J) = F_sens(I,J)
          ELSE             
