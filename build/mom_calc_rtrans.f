...
cph      Real*8 dWtransC(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
cph      Real*8 dWtransU(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
cph      Real*8 dWtransV(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     == Local variables ==
C     I, J :: Loop counters
      INTEGER i,j
CEOP
...
C       anelastic: always assumes that rhoFacF(1) = 1
     ...
     &      0.5D0*( PmEpR( i ,j,bi,bj)*rA( i ,j,bi,bj)
...
C-    Initialise dWtrans :
     ...
     &         *rA(i,j,bi,bj)
        ...
        DO j=1-OLy+1,sNy+OLy
         DO i=1-OLx+1,sNx+OLx
          dWtransU(i,j,bi,bj) =
     &          0.5D0*(dWtransC(i-1,j,bi,bj)+dWtransC(i,j,bi,bj))
          dWtransV(i,j,bi,bj) =
     &          0.5D0*(dWtransC(i,j-1,bi,bj)+dWtransC(i,j,bi,bj))
         ENDDO
        ENDDO
...
C-    Modify rTransU & rTransV :
     ...
     &       + (dWtransC(i-1,j,bi,bj)+dWtransC(i,j,bi,bj))*0.5D0
          rTransV(i,j) = rTransV(i,j)-dWtransV(i,j,bi,bj)
     &       + (dWtransC(i,j-1,bi,bj)+dWtransC(i,j,bi,bj))*0.5D0
         ENDDO
        ENDDO
