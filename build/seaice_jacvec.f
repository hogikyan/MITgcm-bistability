...
C     SEAICEuseTotFluxRestore :: use restoring flux boundary condition for total flux !cyc 
...
C    !cyc: define SEAICEuseTotFluxRestore                               
     &     SEAICEuseTotFluxRestore
...
C     SEAICE_taufluxRest :: restoring strength of total flux (J/m^2/s) !cyc
C     
      Real*8 SEAICE_deltaTtherm, SEAICE_deltaTdyn, SEAICE_deltaTevp
      Real*8 SEAICE_LSRrelaxU, SEAICE_LSRrelaxV
      Real*8 SEAICE_monFreq, SEAICE_dumpFreq
      ...
      Real*8 SEAICE_evpAreaReg, SEAICEpresH0
      ...
      Real*8 SINegFac
C     !cyc: define SEAICE_taufluxRest
      Real*8 SEAICE_taufluxRest
...
C    !cyc: define SEAICE_tausfluxRest
     &    SEAICE_taufluxRest
...
C     Local variables:
      Real*8 utp     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vtp     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
C     u/vIceResP :: residual computed with u/vtp
      Real*8 uIceResP(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 vIceResP(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C
C     1. make small_f
C     2. cat seaice_calc_residual.f seaice_oceandrag_coeffs.f C        seaice_bottomdrag_coeffs.f seaice_calc_stressdiv.f C        seaice_calc_strainrates.f seaice_calc_viscosities.f C        seaice_calc_rhs.f seaice_calc_lhs.f > taf_input.f
C     3. staf -v1 -forward -toplevel seaice_calc_residual C             -input uIceLoc,viceLoc -output uIceRes,vIceRes taf_input.f
...
C
C     1. make small_f
C     2. files="seaice_calc_residual.f seaice_oceandrag_coeffs.f C               seaice_bottomdrag_coeffs.f seaice_calc_stressdiv.f C               seaice_calc_strainrates.f seaice_calc_viscosities.f C               seaice_calc_rhs.f seaice_calc_lhs.f"
C     3. staf -forward -toplevel seaice_calc_residual C             -input uIceLoc,viceLoc -output uIceRes,vIceRes $files
...
C        difference code
CML      Real*8 g_duIce(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
CML      Real*8 g_dvIce(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
CML      Real*8 g_uIceRes(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
CML      Real*8 g_vIceRes(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
CML
CMLC     Initialise
...
CML          g_uIceRes(I,J,bi,bj) = 0.D0
CML          g_vIceRes(I,J,bi,bj) = 0.D0
CML          uIceResP(I,J,bi,bj) = 0.D0
CML          vIceResP(I,J,bi,bj) = 0.D0
CML         ENDDO
CML        ENDDO
CML       ENDDO
CML      ENDDO
