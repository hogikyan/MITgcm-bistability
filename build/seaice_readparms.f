...
CBOP
C     This file was hacked by Malte Jansen to turn off
C     an error if emissivity is set to zero
...
C--   SEAICE parameters
     ...
     & SINegFac,
C     !cyc: define SEAICEuseTotFluxRestore flag and
C     SEAICE_taufluxRest (restoring strength in J/m^2/s)
     & SEAICEuseTotFluxRestore, SEAICE_taufluxRest
...
C     !cyc: SEAICEscaleSurfStress and SEAICEaddSnowMass use new default 
...
C     !cyc: SEAICEuseLinRemapITD use new default
...
C     !cyc: SEAICEetaZmethod and SEAICEadvScheme use new default
      SEAICEetaZmethod   = 3
      SEAICEadvScheme    = 77
...
C     !cyc: SEAICE_OLx and SEAICE_OLy use new default
      SEAICE_OLx          = OLx-2
      SEAICE_OLy          = OLy-2
...
C     Hunke, JCP, 2001 use 615 kg/m^2 for this, but does not recommend using it
      ...
      ICE2WATR           = UNSET_RL
C     !cyc: SEAICE_drag use new default
      SEAICE_drag        = 0.001D0
      OCEAN_drag         = 0.001D0
      SEAICE_waterDrag   = 0.0055D0
      SEAICEsideDrag     = 0.0D0
...
C     !cyc: define SEAICEuseTotFluxRestore flag and
C     SEAICE_taufluxRest (restoring strength in J/m^2/s)
      SEAICEuseTotFluxRestore = .FALSE.
      SEAICE_taufluxRest = 0.0D0
      ...
      ENDDO
C     !cyc: SEAICE_useMultDimSnow use new default
...
C     cyc: comment out error
C     IF ( SEAICE_emissivity .LT. 1.D-04 ) THEN
C       WRITE(msgBuf,'(2A)')
C     &      'SEAICE_emissivity is no longer emissivity*(boltzmann ',
C     &      'constant) but really an emissivity.'
C       CALL PRINT_ERROR( msgBuf , myThid)
C       WRITE(msgBuf,'(2A)')
C     &      'Typical values are near 1 ',
C     &      '(default is 5.5/5.67=0.9700176...).'
C       CALL PRINT_ERROR( msgBuf , myThid)
C       WRITE(msgBuf,'(A,E13.6,A)')
C     &      'Please change SEAICE_emissivity in data.seaice to ',
C     &      SEAICE_emissivity, '/5.67e-8.'
C       CALL PRINT_ERROR( msgBuf , myThid)
