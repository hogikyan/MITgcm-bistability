...
C     i,j,bi,bj,k :: loop indices
      ...
      INTEGER k
      Real*8     SINWAT
      Real*8     COSWAT, recip_deltaT
C     backward difference extrapolation factor
      Real*8 bdfAlpha
C     symmetric drag coefficient
      Real*8 dragSym(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     fractional area at velocity points
      Real*8 areaW(1:sNx,1:sNy)
      Real*8 areaS(1:sNx,1:sNy)
CEOP
...
C     coriols terms: - mass*f*vIce
          uIceLHS(I,J,bi,bj) = uIceLHS(I,J,bi,bj) - 0.5D0*(
     ...
     &           )
C                    + mass*f*uIce
          vIceLHS(I,J,bi,bj) = vIceLHS(I,J,bi,bj) + 0.5D0*(
     &         seaiceMassC(I,J  ,bi,bj) * fCori(I,J  ,bi,bj)
...
C     ocean-ice and bottom drag terms: + (Cdrag*cosWat+Cb)*uIce - vIce*sinWat)
     ...
     &         0.5D0 * ( dragSym(I,J)+dragSym(I-1,J) )
     &         * uIceLoc(I,J,bi,bj)
...
C                                      + (Cdrag*cosWat+Cb)*uIce + uIce*sinWat)
     ...
     &         0.5D0 * ( dragSym(I,J)+dragSym(I,J-1) )
     &         * vIceLoc(I,J,bi,bj)
     &         + SIGN(SINWAT, fCori(I,J,bi,bj)) * 0.5D0 *
     &         (  DWATN(I,J  ,bi,bj) * 0.5D0 *
