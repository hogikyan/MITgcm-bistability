...
C     maskFk    :: 2-D mask for vertical interface k (between level k-1 & k)
      ...
      INTEGER ks
      Real*8 dSigmaH, dSigmaR, Sloc, rTop
c     Real*8 M2loc
      Real*8 recipMaxSlope
      Real*8 deltaH, integrDepth
      Real*8 N2loc, SNloc
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
     ...
     &                  DIAGNOSTICS_IS_ON('GM_KvzTz', myThid )
      ...
      recipMaxSlope = 0.D0
      IF ( GM_Visbeck_maxSlope.GT.0.D0 ) THEN
        recipMaxSlope = 1.D0 / GM_Visbeck_maxSlope
      ENDIF
      DO j=1-OLy,sNy+OLy
       DO i=1-OLx,sNx+OLx
        VisbeckK(i,j,bi,bj) = 0.D0
       ENDDO
      ENDDO
